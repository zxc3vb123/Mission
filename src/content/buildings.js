...
const DATA = [
  ...
  { id: "derrick", timed: true, processing: true, storage: 400, name: "Derrick", w: 18, h: 48,
    ...
    support: { ground: 0.5, indoors: false }, stage: 5,
    enables: "Sinking and holding a bore down to an oil pocket, and a tank at the foot of it for what comes up.",
    note: "GROUND 0.5, WHICH IS THE ONE ENTRY IN THIS TABLE THAT MUST NOT ASK FOR SOLID GROUND. A derrick is four legs around a hole; standing on unbroken earth is exactly what it does not do. At 1.0 the tower and its own well were mutually exclusive - lane D found a derrick placeable on clean ground and refused the moment a seven-pixel bore was cut under it, and a structure already standing came down when the shaft was sunk. Correctly: nothing floats. Over an 18 px footprint, 0.5 admits a bore up to 9 px, which is comfortable room to aim in; 0.6 admits 7 and would refuse a slightly wide hole with a message that reads like a bug. The kiln and the forge stay at 1.0 because masonry sits on its footprint. The honest limitation: a fraction is the only predicate placement has, so this also lets a derrick stand half over a cliff. What it really wants is LEGS on solid ground rather than a share of its width, and if that predicate ever exists this should use it. TANK: 400 kg is 33 measures of crude, which is exactly three barrels - about 25 minutes of pumping to fill and jam, which is the real cadence at which a player has to come back. TIMBER AND ROPE, NO METAL, at lane D's ask and it is the right call - the same argument that put the sawmill on wood, stone and rope. What gates oil is not the tower, it is finding the stuff: oil is a middle-band material, so you need an iron pickaxe to reach the depth at all. The gate is the tool tier, and a cheap derrick on top of an expensive tool reads better than the reverse. PROGRESSION said steel and pipe; it was written before the tier system existed and I have corrected it." },
